...
from app.schemas.schemas import (
    AnomalyScoreRequest, AnomalyScoreResponse, AssetCreate, HoldingCreate, MarketPriceCreate,
    PortfolioCreate, PortfolioRead, TokenResponse, TransactionCreate, TransactionRead, UserCreate,
)
...
def login(email: str, password: str, db: Session = Depends(get_db)):
    ...
    return TokenResponse(access_token=create_access_token(user.email, str(user.role.value)))
...
def add_holding(portfolio_id: int, request: HoldingCreate, db: Session = Depends(get_db)):
...
def create_market_price(request: MarketPriceCreate, db: Session = Depends(get_db)):
    asset = db.query(Asset).filter(Asset.symbol == request.symbol).first()
    if not asset:
        asset = Asset(symbol=request.symbol, asset_type="equity", exchange="SIM", currency="USD")
        db.add(asset)
        db.commit()
        db.refresh(asset)
    ...
    db.commit()
    db.refresh(price)
    return {"symbol": request.symbol, "price": request.price, "event_time": price.event_time}
...
def score_anomaly(request: AnomalyScoreRequest):
    score, is_anomaly = anomaly_detector.score([
        request.amount, request.quantity, request.price, request.hour_of_day, request.account_age_days
    ])
