...
def health():
    ...
    return {"status": "healthy", "service": "finanalytics-api"}
...
def register(req: schemas.RegisterRequest, db: Session = Depends(get_db)):
    ...
    u = models.User(
        email=req.email, hashed_password=hash_password(req.password), role=req.role
    )
    db.add(u)
    db.commit()
    db.refresh(u)
...
def add_holding(
    portfolio_id: int, req: schemas.HoldingCreate, db: Session = Depends(get_db)
):
    h = models.Holding(portfolio_id=portfolio_id, **req.model_dump())
    db.add(h)
    db.commit()
    db.refresh(h)
    publish("portfolio.updated", {"portfolio_id": portfolio_id, "holding_id": h.id})
    return h
...
def performance(portfolio_id: int, db: Session = Depends(get_db)):
    value = portfolio_value(db, portfolio_id)
    risk = (
        db.query(models.RiskMetric)
        .filter_by(portfolio_id=portfolio_id)
        .order_by(desc(models.RiskMetric.calculated_at))
        .first()
    )
    return {
        "portfolio_id": portfolio_id,
        "current_value": value,
        "daily_change_pct": round(random.uniform(-2.2, 2.8), 2),
        "risk": risk,
    }
...
def create_transaction(req: schemas.TransactionCreate, db: Session = Depends(get_db)):
    ...
    sc = anomaly_score(amount, req.quantity, req.price, random.randint(1, 12))
    ...
    db.commit()
    publish("transactions.created", payload)
    return tx
...
def price(req: schemas.MarketPriceCreate, db: Session = Depends(get_db)):
    mp = models.MarketPrice(**req.model_dump())
    db.add(mp)
    db.commit()
    db.refresh(mp)
    publish("market.prices.updated", req.model_dump())
    return mp
...
def simulate_market_events(count: int = 10, db: Session = Depends(get_db)):
    ...
    for _ in range(count):
        ...
        new = round((last.price if last else 100) * random.uniform(0.985, 1.018), 2)
        mp = models.MarketPrice(asset_id=a.id, symbol=a.symbol, price=new)
        db.add(mp)
        out.append({"symbol": a.symbol, "price": new})
    db.commit()
    publish("market.prices.updated", {"count": count, "prices": out})
    return {"generated": count, "prices": out}
...
def prices(db: Session = Depends(get_db)):
    ...
    for a in assets:
        ...
        res.append({"symbol": a.symbol, "price": p.price if p else None})
    return res


@app.get("/api/v1/risk/portfolio/{portfolio_id}")
def risk(portfolio_id: int, db: Session = Depends(get_db)):
    return calculate_risk(db, portfolio_id)


@app.get("/api/v1/alerts")
def alerts(db: Session = Depends(get_db)):
    return (
        db.query(models.Alert).order_by(desc(models.Alert.created_at)).limit(50).all()
    )


@app.post("/api/v1/anomaly/score")
def score(req: schemas.AnomalyScoreRequest):
    return {
        "anomaly_score": anomaly_score(
            req.amount, req.quantity, req.price, req.transaction_frequency_1h
        )
    }
...
def demo_events(count: int = 15, db: Session = Depends(get_db)):
    ...
    for _ in range(count):
        ...
        tx = models.Transaction(
            ...
            amount=round(amount, 2),
            status="processed",
            anomaly_score=anomaly_score(amount, qty, price, random.randint(1, 15)),
        )
        db.add(tx)
        db.flush()
    ...
    db.commit()
    calculate_risk(db, p.id)
    return {"generated_transaction_ids": generated}
...
def dashboard(db: Session = Depends(get_db)):
    ...
    alerts = (
        db.query(models.Alert).order_by(desc(models.Alert.created_at)).limit(10).all()
    )
    risk = (
        db.query(models.RiskMetric)
        .order_by(desc(models.RiskMetric.calculated_at))
    ...
    return {
        "status": "online",
        "portfolio_value": portfolio_value(db, p.id) if p else 0,
